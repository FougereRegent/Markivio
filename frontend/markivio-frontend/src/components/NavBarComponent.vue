...
import { ArticleTypeFiltering, useArticleStats } from '@/features/article/composables/article.graphql';
    ...
    items: [
      {
        ...
        badge: computed(() => stats.value?.numberOfAllArticle.totalCount ?? 0),
        command: () => changeTypeFilter(ArticleTypeFiltering.all)
      },
      {
        label: t("nav.favorites"),
        icon: "ri-star-line",
        badge: computed(() => stats.value?.numberOfFavoriteArticle.totalCount ?? 0),
        command: () => changeTypeFilter(ArticleTypeFiltering.favorite)
      },
      {
        label: t("nav.toRead"),
        icon: "ri-book-line",
        badge: computed(() => stats.value?.numberOfNewArticle.totalCount ?? 0),
        command: () => changeTypeFilter(ArticleTypeFiltering.new)
      },
      {
        label: t("nav.archived"),
        icon: "ri-archive-line",
        badge: computed(() => stats.value?.numberOfReadArticle.totalCount ?? 0),
        command: () => changeTypeFilter(ArticleTypeFiltering.archived)
      },
    ]
  },
  {
  ...
  <Menu :model="items" class="w-full h-full flex flex-col justify-between" :pt="{
    ...
    <template #submenulabel="{ item }">
      <div class="flex flex-row justify-between w-full h-full px-4 py-2">
        ...
        <i class="text-gray-400 text-xl ml-auto hover:opacity-70" :class="item.icon"
          @click="(item.command as any)?.()" />
      </div>
    </template>
    <template #item="{ item, props }">
      <a v-ripple class="flex items-center px-4 py-2.5" v-bind="props.action">
        <div class="flex flex-row gap-3 justify-start items-center w-full">
          ...
          <BadgeComponent :counter="item.badge" v-if="item.badge" />
        </div>
      </a>
    </template>
    <template #end>
      <div class="border-t border-neutral-200">
        <div class="flex items-center gap-3 cursor-pointer hover:bg-neutral-50 transition-colors px-4 py-3"
          @click="clickUserIcon">
          <UserIconComponent />
