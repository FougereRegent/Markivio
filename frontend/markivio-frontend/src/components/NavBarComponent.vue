  ...
  <Menu :model="items" class="md:w-60 h-full flex flex-col justify-between" :pt="{
    ...
    <template #submenulabel="{ item }">
      <div class="flex flex-row justify-between w-full h-full">
        ...
        <i class="text-gray-400 text-2xl ml-auto" :class="item.icon" @click="item.command" />
      </div>
    </template>
      ...
      <a v-ripple class="flex items-center" v-bind="props.action">
        <div class="flex flex-row gap-4 justify-start items-center w-full">
          ...
          <i class="text-gray-400 text-2xl" :class="item.icon" v-if="item.icon" />
            <span v-tooltip.right="{value: item.label, showDelay: 650}" class="text-xl truncate w-35">{{ item.label }}</span>
